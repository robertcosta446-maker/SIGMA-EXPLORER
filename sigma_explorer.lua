...
Main = (function()
	...
	Main.Apps = Apps
	Main.MenuApps = {}
	Main.GitRepoName = "sigma-explorer"
	...
	Main.Error = function(str)
		if rconsoleprint then
			rconsoleprint("SIGMA ERROR: "..tostring(str).."\n")
			wait(9e9)
		else
			error(str)
		end
	...
	Main.LoadModule = function(name)
		if Main.Elevated then -- If you don't have filesystem api then ur outta luck tbh
			...
			elseif _G.DebugLoadModel then -- Load Debug Model File
				...
				local filePath = "sigma/ModuleCache/"..name..".lua"
	...
	Main.LoadSettings = function()
		local s, data = pcall(env.readfile or error, "SigmaSettings.json")
	...
	Main.FetchAPI = function(callbackiflong, callbackiftoolong, XD)
		...
		if Main.Elevated then
			if Main.LocalDepsUpToDate() then
				local localAPI = Lib.ReadFile("sigma/rbx_api.dat")
				if localAPI then 
					rawAPI = localAPI
				else
	...
	Main.FetchRMD = function()
		...
		if Main.Elevated then
			if Main.LocalDepsUpToDate() then
				local localRMD = Lib.ReadFile("sigma/rbx_rmd.dat")
				if localRMD then 
					rawXML = localRMD
				else
					Main.DepsVersionData[1] = ""
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{5,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=4,Name="Title",Parent={3},Position=UDim2.new(0,-190,0,15),Size=UDim2.new(0,100,0,50),Text="Sigma",TextColor3=Color3.new(1,1,1),TextSize=50,TextTransparency=1,}},
			{6,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Desc",Parent={3},Position=UDim2.new(0,-230,0,60),Size=UDim2.new(0,180,0,25),Text="Sigma Debugging Suite",TextColor3=Color3.new(1,1,1),TextSize=18,TextTransparency=1,}},
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Sigma Explorer",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
	...
	Main.SetMainGuiOpen = function(val)
		...
		Main.MainGui.OpenButton.Text = val and "Close" or "Sigma"
	...
	Main.SetupFilesystem = function()
		...
		makefolder("sigma")
		makefolder("sigma/assets")
		makefolder("sigma/saved")
		makefolder("sigma/plugins")
		makefolder("sigma/ModuleCache")
	end
	...
	Main.Init = function()
		...
		if writefile and isfile and not isfile("SigmaSettings.json") then
			writefile("SigmaSettings.json", Main.ExportSettings())
		end
		...
		if Main.Elevated then
			local fileVer = Lib.ReadFile("sigma/deps_version.dat")
		...
		if Main.Elevated and env.writefile and not Main.LocalDepsUpToDate() then
			env.writefile("sigma/deps_version.dat",Main.ClientVersion.."\n"..Main.RobloxVersion)
			env.writefile("sigma/rbx_api.dat",Main.RawAPI)
			env.writefile("sigma/rbx_rmd.dat",Main.RawRMD)
		end
